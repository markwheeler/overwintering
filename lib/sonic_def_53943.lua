-- Ruff

local MusicUtil = require "musicutil"

SonicDef = {

    -- Key
    musical_scale = MusicUtil.generate_scale(33, "Major", 8), -- A0
  
    -- Defines where the roots are measured from
    chord_start_x = 0.2, -- 0 is left, 1 is right
    chord_start_y = 0.4, -- 0 is top, 0.5 is bottom
    perc_start_x = 0.12,
    perc_start_y = 0.3,
  
    -- When to expand vs contract trigger order
    contract_range = {35, 3},
  
    -- Dynamic params
    dynamic_params = {
  
      -- Chord
      chord_osc_wave_shape_low = 1,
      chord_osc_wave_shape_high = 0.25,
      chord_noise_level_low = 0,
      chord_noise_level_high = 0.01,
      chord_lp_filter_cutoff_low = 500,
      chord_lp_filter_cutoff_high = 1500,
      chord_amp_mod_lfo_low = 0.5,
      chord_amp_mod_lfo_high = 0.05,
      chord_lfo_freq_low = 0.01,
      chord_lfo_freq_high = 0.0015,
      chord_env_decay_low = 2,
      chord_env_decay_high = 2,
      chord_env_release_low = 4,
      chord_env_release_high = 5,
      chord_amp_low = 0.8,
      chord_amp_high = 1.1,
  
      -- Perc
      perc_osc_level_low = 0.25,
      perc_osc_level_high = 0.9,
      perc_sub_osc_level_low = 0,
      perc_sub_osc_level_high = 0.3,
      perc_crackle_level_low = 0.1,
      perc_crackle_level_high = 0,
      perc_env_release_low = 2,
      perc_env_release_high = 0.5,
      perc_panning_low = -0.94,
      perc_panning_high = 0.94,
      perc_lp_filter_cutoff_low = 120,
      perc_lp_filter_cutoff_high = 350,
      perc_lp_filter_resonance_low = 0,
      perc_lp_filter_resonance_high = 0,
      perc_lfo_freq_low = 0.05,
      perc_lfo_freq_high = 25,
      perc_delay_send_low = 0.2,
      perc_delay_send_high = 0.05,
  
      -- FX
      fx_delay_feedback_low = 0.2,
      fx_delay_feedback_high = 0.9
    },
  
    -- Params
    params = {
  
      -- Chord
      chord_detune_variance = 0.4,
      chord_freq_mod_env = 0,
      chord_freq_mod_lfo = 0,
      chord_osc_wave_shape_mod_env = 0,
      chord_osc_wave_shape_mod_lfo = 0,
      chord_osc_level = 1,
      
      chord_lp_filter_cutoff_mod_env = 0,
      chord_lp_filter_cutoff_mod_lfo = 0.05,
      chord_lp_filter_resonance = 0.2,
  
      chord_env_attack = 5,
      chord_env_sustain = 1,
  
      chord_ring_mod_mix = 0,
      chord_ring_mod_mix_env = 0,
      chord_ring_mod_mix_lfo = 0.2,
      chord_ring_mod_freq = 223,
  
      chord_chorus_send = 1,
      chord_delay_send = 0.2,
  
      -- Perc
      perc_detune_variance = 0.2,
      perc_freq_mod_env = 0,
      perc_freq_mod_lfo = 0,
  
      perc_osc_wave_shape = 1,
      perc_osc_wave_shape_mod_env = 0,
      perc_osc_wave_shape_mod_lfo = 0.15,
      perc_noise_level = 0,
  
      perc_lp_filter_cutoff_mod_env = 0.05,
      perc_lp_filter_cutoff_mod_lfo = 0,
  
      perc_env_attack = 0.01,
      perc_amp = 1,
      perc_amp_mod_lfo = 0.3,
  
      perc_chorus_send = 0.25,
  
      -- FX
      fx_delay_time = 0.88,
      fx_delay_mod_freq = 0.2,
      fx_delay_mod_depth = 0.2
    }
  }

  return SonicDef
  